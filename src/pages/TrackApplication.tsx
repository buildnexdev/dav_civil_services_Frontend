            ...
            <div className="card track-form-card">
              ...
              <div className="form-grid">
                <div className="form-group">
                  ...
                  <input className="form-control" value={appId} onChange={(e) => setAppId(e.target.value)} placeholder="e.g. APP2025001" />
                </div>
                <div className="form-group">
                  <label>Date of Birth / Mobile</label>
                  <input className="form-control" value={dob} onChange={(e) => setDob(e.target.value)} placeholder="DD/MM/YYYY or Mobile" />
                </div>
              </div>
              <button className="btn btn-primary mt-4" onClick={handleTrack}>Track</button>
            </div>
          ) : (
            <div className="card track-result-card">
              ...
              <div className="timeline">
                {timelineSteps.map((s, i) => (
                  <div className={`timeline-step ${i <= currentStep ? 'completed' : ''} ${i === currentStep ? 'current' : ''}`} key={s}>
                    <div className="tl-dot">{i <= currentStep ? '✓' : i + 1}</div>
                    <div className="tl-content">
                      <div className="tl-title">{s}</div>
                      {i <= currentStep && <div className="tl-date">Completed</div>}
                    </div>
                  </div>
                ))}
              </div>
